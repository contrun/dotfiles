...
let
  ...
  hardwareConfiguration = builtins.trace (hostname) (if isMinimalSystem then
    import (pathOr (myNixConfigPath "hardware-configuration-${hostname}.nix")
      (myNixConfigPath "hardware-configuration-fake.nix"))
  else
    import (pathOr (myNixConfigPath "hardware-configuration-${hostname}.nix")
  ...
  myOverlaysConfiguration = {
    nixpkgs.overlays = let
      ...
      overrideOutputsToInstall = attr: outputs:
        if !(builtins.isAttrs attr && attr ? meta && attr ? overrideAttrs
          && attr ? outputs) then
          attr
        else
          let
      ...
      myOverlay = self: super: {
        ...
        in rec {
          ...
          texLive =
            self.texlive.combine { inherit (self.texlive) scheme-full; };
